...
export default function ConsultantaProiecte() {
  ...
  useEffect(() => {
    fetch("/api/page/consultanta-in-accesarea-si-implementarea-proiectelor")
      .then((res) => res.json())
      .then((data) => {
              // Eliminăm doar cuvântul "Contact" din titlu dacă apare în content
              const cleanedContent = data.content.replace(/<h[1-3]>.*?Consultanță în accesarea și implementarea proiectelor.*?<\/h[1-3]>/gi, "");
